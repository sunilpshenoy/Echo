...
def setup():
    global user1_token, user2_token, user1_id, user2_id, test_chat_id, ws_client1, ws_client2, test_team_id, test_team_chat_id
...
def test_custom_emoji_upload_and_management():
    print("\n=== Testing Custom Emoji Upload and Management ===")
    ...
    print("All custom emoji upload and management tests passed!")
...
def test_gif_upload_and_messaging():
    ...
    files = {'file': (gif_filename, gif_buffer, 'image/gif')}
    response = requests.post(f"{API_URL}/upload", headers=headers, files=files)
    assert response.status_code == 200, f"Failed to upload GIF: {response.text}"
    ...
    gif_message_id = gif_message["message_id"]
    ...
    reaction_data = {"emoji": "👍"}
    response = requests.post(
        f"{API_URL}/messages/{gif_message_id}/reactions", 
        json=reaction_data, 
        headers=headers
    )
...
def cleanup():
    ...
    if 'ws_client1' in globals() and ws_client1:
        ws_client1.disconnect()
    if 'ws_client2' in globals() and ws_client2:
