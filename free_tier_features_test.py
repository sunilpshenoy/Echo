...
test_users = [
    {
        "username": "alice_free_tier",
        "email": "alice.free.tier@example.com",
        "password": "SecurePass123!",
        "phone": "+1234567890",
        "display_name": "Alice Free Tier"
    },
    {
        "username": "bob_free_tier",
        "email": "bob.free.tier@example.com",
        "password": "StrongPass456!",
        "phone": "+1987654321",
        "display_name": "Bob Free Tier"
    }
]
...
def test_voice_video_calls():
    ...
    if user_ids['user1'] not in response.json().get("participants", []) or user_ids['user2'] not in response.json().get("participants", []):
        logger.error(f"Call participants list is incorrect: {response.json().get('participants')}")
        return False
...
def test_file_sharing():
    ...
    # Verify file data is correctly stored
    if (response.json().get("message_type") != "file" or 
        "file_data" not in response.json() or
        response.json()["file_data"].get("filename") != "test_image.png"):
        logger.error(f"File data not correctly stored in message: {response.json()}")
        return False
    
    # Create a text file
    text_data = "This is a test text file for file sharing"
    ...
    file_data = {
        ...
        "size": len(text_data),
        "type": "text/plain",
        "data": base64.b64encode(text_data.encode('utf-8')).decode('utf-8')
    }
    ...
    if response.status_code != 200:
        logger.error(f"Failed to upload text file to chat: {response.text}")
        return False
    
    logger.info(f"Successfully uploaded text file to chat")
...
def test_teams_api():
    ...
    # Create another team with different data
    team_data = {
        "name": f"Another Team {uuid.uuid4().hex[:8]}",
    ...
    second_team_id = response.json().get("team_id")
    logger.info(f"Successfully created second team with ID: {second_team_id}")
...
def run_tests():
    ...
    overall_result = voice_video_result and file_sharing_result and teams_result
